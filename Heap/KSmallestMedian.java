...
public class KSmallestMedian{
	...
	public static int RfindRankK(int[] a,int i,int j,int r,int lessThanOrNot){
		int p=random(i,j);
		//out.println("p is "+p);
		int k=partition(a,i,j,p);

		if(r>=j-k+1 && lessThanOrNot==1){
				return k;
			//return a[k];
		}else{
			if(r==j-k+1 && lessThanOrNot==0){
				return k;
			}
			if(r<j-k+1){
				//out.println("\nR< j:"+j+" k:"+k+" r:"+r);
				return RfindRankK(a,k+1,j,r,lessThanOrNot);
			}else{
				//out.println("\nR> j:"+j+" k:"+k+" r:"+r);
				return RfindRankK(a,i,k-1,r-(j-k),lessThanOrNot);
			}
		}
	}
